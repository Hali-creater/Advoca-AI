document.addEventListener('DOMContentLoaded', () => {
    ...
    const handleConversationFlow = (text) => {
        if (conversationState === 'idle') {
            speak("Welcome to Advoca AI. I am an AI Receptionist Assistant. Please select the type of law case you have.");
            displayLawTypeButtons();
            conversationState = 'awaiting_law_type';
            return;
        }

        if (conversationState === 'awaiting_law_type') {
            if (LAW_QUESTIONS[text]) {
                currentLawType = text;
                tempLeadData['law_type'] = currentLawType;
                askNextQuestion();
                conversationState = 'asking_questions';
            } else {
                speak("Please select a valid law type.");
                displayLawTypeButtons();
            }
            return;
        }

        if (conversationState === 'asking_questions') {
            const questions = LAW_QUESTIONS[currentLawType];
            const questionKey = questions[currentQuestionIndex].key;
            tempLeadData[questionKey] = text;
    ...
    const displayLawTypeButtons = () => {
        const buttonContainer = document.createElement('div');
        buttonContainer.classList.add('button-container');
        for (const lawType in LAW_QUESTIONS) {
            const button = document.createElement('button');
            button.textContent = lawType;
            button.addEventListener('click', () => {
                processUserInput(lawType);
